...
function ContentUpload1_1A({ onComplete, onBack }) {
  ...
  useEffect(() => {
    logScreenView('content_upload_1_1a');
    ...
    return () => {
      ...
      logScreenExit('content_upload_1_1a', dwellTime);
    };
  }, []);
  ...
  const handleTimelineCutSelect = (index) => {
    logButtonClick('content_upload_1_1a', 'timeline_cut_select', String(index + 1));
    setAiSuggestions([]);
  ...
  // 뒤로가기
  const handleBack = () => {
    logButtonClick('content_upload_1_1a', 'back');
    onBack();
  };
  ...
  const handleVideoUpload = (e) => {
    ...
    if (file) {
      logButtonClick('content_upload_1_1a', 'video_upload');
  ...
  // 영상 영역 클릭 — 영상이 있으면 재생, 없으면 파일 선택
  const handleVideoAreaClick = () => {
    if (currentCut.videoPreview && videoRef.current) {
      logButtonClick('content_upload_1_1a', 'play_video');
  ...
  const handleAISubtitle = async () => {
    logButtonClick('content_upload_1_1a', 'ai_subtitle');
    setIsLoadingAI(true);
    setAiSuggestions([]);
    setSelectedSuggestionIndex(null);
  ...
  const handleSelectSuggestion = (suggestion, index) => {
    logButtonClick('content_upload_1_1a', 'ai_suggestion_select', suggestion);
  ...
  // "완성하기" — 로그만 남기고, 완료 처리는 영상 업로드+재생 시에만 동작
  const handleComplete = () => {
    logButtonClick('content_upload_1_1a', 'complete');
  };
    ...
    <div className="cua-container">
      <div className="cua-scroll-content">
        ...
        <div className="cua-main-preview">
          ...
          {/* 영상 추가 영역 - 영상이 있으면 클릭 시 재생, 없으면 파일 선택 */}
          <div className="cua-video-area" onClick={handleVideoAreaClick}>
...
export default ContentUpload1_1A;
